...
//        System.out.println(calculateWithPriority("(((2+(30/10)+(11-1)*2)))"));
    ...
    public static int calculateWithPriority(String s) {
        ...
        int[] ansAndEndIndex = calculateWithPriority(s, 0, stack);
        return ansAndEndIndex[0] + calculateWithPriority(s, ansAndEndIndex[1] + 1, stack)[0];
    }

    private static int[] calculateWithPriority(String s, int from, Stack<String> stack) {

        if (from > s.length() - 1) return new int[]{0, from};

        if (s.charAt(from) == '(') {
            return calculateWithPriority(s, from + 1, stack);
        }
        ...
        for (; i <= s.length(); i++) {
            ...
            if (i < s.length() && s.charAt(i) == '(') {
                int[] priority = calculateWithPriority(s, i + 1, stack);
                cur = priority[0];
                i = priority[1] + 1;
            }
            ...
            if (i < s.length() && s.charAt(i) >= '0' && s.charAt(i) <= '9') {
                ...
                if (!stack.isEmpty() &&
                    ...
                    if (!(i == from && cur == 0)) stack.push(String.valueOf(cur));
                }
